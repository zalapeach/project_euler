...
class LargestProductGrid
  ...
  def line_up(i, j)
    ...
    result = 0
    if (i - @adjacents + 1) >= 0
      ((i - @adjacents + 1)..i).each do |lateral|
        values << @grid[lateral][j]
      end
  ...
  def line_right(i, j)
    ...
    result = 0
    if (j + @adjacents - 1) <= 20
      ((j + @adjacents - 1)..j).each do |horizontal|
  ...
  def welcome
    puts "Get Largest Product on Grid"
    print "Adjacent: "
    STDOUT.flush
    gets.chomp.to_i
  end
